...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-300 font-sans antialiased selection:bg-amber-500/30 selection:text-amber-200">
      
      {/* GLORIOUS TOP DECORATIVE LOGO & NAVIGATION */}
      <header className="bg-zinc-900/80 backdrop-blur-md border-b border-zinc-800 sticky top-0 z-40 shadow-xl/50 shadow-black/40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 space-y-3">
          ...
          <nav className="flex flex-wrap items-center gap-2 bg-zinc-900/60 p-1.5 rounded-xl border border-zinc-800 w-full">
            <button
              onClick={() => {
                setEditPMCode("");
                setActiveTab("inicio");
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {overdueList.length > 0 && (
          <motion.div
            ...
            className="bg-red-950/20 border-l-4 border-red-500 p-5 rounded-xl shadow-md border border-red-900/40 space-y-3"
          >
              ...
              {overdueList.map(({ pm, failure }) => {
                return (
                  <div
                    ...
                    className="bg-zinc-900/60 p-3.5 rounded-lg border border-zinc-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 shadow-sm transition-all hover:bg-zinc-900"
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-mono font-bold text-zinc-100 text-sm">{pm.codigoPM}</span>
                        <span className="text-xs text-zinc-400">({pm.nombrePM})</span>
                        <span className="bg-red-500/20 text-red-400 border border-red-500/35 text-[9px] font-bold px-1.5 py-0.5 rounded-full uppercase">
                          {pm.estadoGral}
                        </span>
                      </div>

                      <p className="text-xs text-zinc-300">
                        Falla en <strong className="text-zinc-100">{failure.componente}</strong> ({failure.detalle}).
                        Debió gestionarse el: <strong className="text-red-400">{failure.fechaReparacion}</strong>
                      </p>
                    </div>
        ...
        {loading ? (
          <div className="text-center py-24">
            ...
            <p className="text-sm text-zinc-400 font-medium">Sincronizando con base de datos en la nube...</p>
          </div>
        ) : (
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-xs text-zinc-400 font-medium">Puntos de medida activos en falla, instalación o revisión</p>
                    </div>
                  </div>
      ...
      {reprogramTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/85 backdrop-blur-sm p-4">
          <div className="bg-zinc-900 w-full max-w-sm rounded-xl shadow-xl border border-zinc-800 p-5 space-y-4">
            
            <div className="flex justify-between items-center border-b border-zinc-800 pb-2">
              <h3 className="text-sm font-extrabold text-amber-400 flex items-center gap-1.5 uppercase tracking-wider">
            ...
            <div className="space-y-1">
              <p className="text-xs text-zinc-400 font-medium">PM: <strong className="text-zinc-200 font-mono">{reprogramTarget.pm.codigoPM}</strong></p>
              <p className="text-xs text-zinc-400 font-medium">Componente: <strong className="text-zinc-200">{reprogramTarget.failure.componente} ({reprogramTarget.failure.detalle})</strong></p>
            </div>

            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-zinc-300">Nueva Fecha de Programación</label>
              <input
                type="date"
                value={reprogramDate}
                onChange={(e) => setReprogramDate(e.target.value)}
                className="w-full px-3 py-2 text-sm bg-zinc-950 border border-zinc-800 text-zinc-100 rounded-lg focus:outline-none focus:border-amber-500"
                required
              />
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-zinc-800">
              ...
              <button
                ...
                className="px-4 py-1.5 bg-amber-500 hover:bg-amber-400 text-zinc-950 text-xs font-semibold rounded-lg shadow-sm cursor-pointer disabled:bg-zinc-800 disabled:text-zinc-600 disabled:cursor-not-allowed"
              >
                Actualizar Fecha
              </button>
            </div>
      ...
      {/* FOOTER */}
      <footer className="py-8 bg-zinc-950/50 text-center text-xs text-zinc-500 border-t border-zinc-900 mt-12">
        <p>© 2026 Cinco S.A.S</p>
        <p className="mt-1 font-medium text-[10px] text-zinc-600">Construido para Gestión Profesional de Redes de Medida</p>
      </footer>
