...
export default function Dashboard({ pms, baseData, onNavigate }: DashboardProps) {
  ...
  const ChartCustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div 
          className="bg-zinc-950/95 border border-zinc-800 rounded-xl p-3 shadow-xl max-w-xs text-xs font-sans space-y-2 backdrop-blur-md cursor-pointer hover:border-amber-500/40 transition-all"
        ...
        >
          ...
          <div className="flex items-center justify-between text-[10px] text-zinc-300 bg-zinc-900/80 px-2 py-1 rounded-md font-mono">
            <span>Fallas: <strong className="text-red-400">{data["Fallas del Mes"]}</strong></span>
            <span>Corregidos: <strong className="text-emerald-400">{data["Arreglados del Mes"]}</strong></span>
            <span>Acumulado: <strong className="text-amber-400">{data["Balance Acumulado"]}</strong></span>
          </div>

          {/* Listado sobrio de puntos de medida */}
          <div className="space-y-1 pt-0.5">
            <p className="text-[10px] font-extrabold uppercase text-zinc-500 tracking-wider">
              Puntos con Falla:
            </p>
            {fallas.length === 0 ? (
              <p className="text-[10px] text-zinc-500 italic">Sin fallas registradas</p>
            ) : (
              <div className="space-y-1">
                {fallas.slice(0, 3).map((item: any, idx: number) => (
                  <div key={idx} className="flex items-center justify-between text-[11px] py-0.5 border-b border-zinc-900/60 last:border-none">
                    <span className="font-mono font-bold text-amber-400 truncate max-w-[100px]">
                      {item.codigoPM}
                    </span>
                    <span className="text-[10px] text-zinc-400 truncate max-w-[140px] text-right">
                      {item.nombrePM}
                    </span>
                  </div>
                ))}
                {fallas.length > 3 && (
                  <p className="text-[10px] text-amber-400 font-bold pt-1 text-center">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-zinc-900/60 backdrop-blur-md border border-zinc-800 rounded-2xl p-4 shadow-xl">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400 shrink-0">
            <Activity className="w-5 h-5 text-amber-400" />
          </div>
          <div>
            <h2 className="text-sm sm:text-base font-extrabold text-zinc-100 uppercase tracking-wider">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-zinc-900/40 backdrop-blur-md border border-zinc-800 rounded-xl p-4.5 shadow-lg flex items-center gap-4 hover:border-red-500/40 transition-all group">
          <div className="w-12 h-12 rounded-xl bg-red-500/15 border border-red-500/25 flex items-center justify-center text-red-400 group-hover:scale-105 transition-transform shrink-0">
            <AlertTriangle className="w-6 h-6 text-red-400" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[11px] text-zinc-500 uppercase tracking-wider font-extrabold">Puntos en Falla</p>
            <div className="flex flex-col gap-1 mt-1">
              <div className="flex items-baseline gap-2 flex-wrap">
                <span className="text-3xl font-black text-zinc-100">{pmsInFailure}</span>
                <span className="text-sm sm:text-base font-black text-red-400 bg-red-500/15 border border-red-500/30 px-2.5 py-0.5 rounded-lg shadow-sm">
                  {affectedPercentage}%
                </span>
              </div>
        ...
        <div className="bg-zinc-900/40 backdrop-blur-md border border-zinc-800 rounded-xl p-4.5 shadow-lg flex items-center gap-4 hover:border-blue-500/40 transition-all group">
          <div className="w-12 h-12 rounded-xl bg-blue-500/15 border border-blue-500/25 flex items-center justify-center text-blue-400 group-hover:scale-105 transition-transform shrink-0">
            <Settings className="w-6 h-6 text-blue-400" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[11px] text-zinc-500 uppercase tracking-wider font-extrabold">Para Instalación</p>
            <div className="flex items-baseline gap-2 mt-1">
              <span className="text-3xl font-black text-zinc-100">{pmsInInstallation}</span>
            </div>
        ...
        <div className="bg-zinc-900/40 backdrop-blur-md border border-zinc-800 rounded-xl p-4.5 shadow-lg flex items-center gap-4 hover:border-purple-500/40 transition-all group">
          <div className="w-12 h-12 rounded-xl bg-purple-500/15 border border-purple-500/25 flex items-center justify-center text-purple-400 group-hover:scale-105 transition-transform shrink-0">
            <CheckCircle className="w-6 h-6 text-purple-400" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[11px] text-zinc-500 uppercase tracking-wider font-extrabold">Puntos Revisados</p>
            <div className="flex items-baseline gap-2 mt-1">
              <span className="text-3xl font-black text-zinc-100">{pmsRevisados}</span>
            </div>
            <span className="text-[11px] text-zinc-400 font-medium block mt-1">
        ...
        <div className="bg-zinc-900/40 backdrop-blur-md border border-zinc-800 rounded-xl p-4.5 shadow-lg flex flex-col justify-between gap-3 hover:border-amber-500/40 transition-all group">
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-start gap-3">
              <div className="w-11 h-11 rounded-xl bg-amber-500/15 border border-amber-500/25 flex items-center justify-center text-amber-400 group-hover:scale-105 transition-transform shrink-0">
                <MapPin className="w-5 h-5 text-amber-400" />
              </div>
              <div>
                <p className="text-[11px] text-zinc-500 uppercase tracking-wider font-extrabold">Estructuras Afectadas</p>
                <div className="flex flex-col gap-1 mt-1">
                  <div className="flex items-baseline gap-2 flex-wrap">
                    <span className="text-3xl font-black text-zinc-100">{totalAffectedStructures}</span>
                    <span className="text-sm sm:text-base font-black text-amber-400 bg-amber-500/15 border border-amber-500/30 px-2.5 py-0.5 rounded-lg shadow-sm">
            ...
            <button
              ...
              className="px-2.5 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-amber-400 text-xs font-extrabold transition-all flex items-center gap-1 shrink-0 cursor-pointer shadow-sm"
          ...
          <div className="flex flex-wrap items-center gap-1.5 pt-2 border-t border-zinc-800/60">
            {Object.keys(typeCounts).length === 0 ? (
              <span className="text-[10px] text-zinc-500 font-medium">Sin estructuras afectadas</span>
            ) : (
              Object.entries(typeCounts).map(([typeKey, count]) => (
                <span
                  key={typeKey}
                  className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-md bg-zinc-800/80 border border-zinc-700/80 text-zinc-300 flex items-center gap-1"
                >
                  <span className="text-amber-400 font-black">{typeKey}:</span>
                  <span>{count}</span>
                </span>
              ))
            )}
      ...
      {showStructuresDropdown && (
        <div className="bg-zinc-900/50 border border-amber-500/30 rounded-2xl p-5 shadow-xl space-y-4 animate-fade-in">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-zinc-800 pb-3">
            <div>
              <h3 className="text-sm font-extrabold text-zinc-100 uppercase tracking-wider flex items-center gap-2">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <button
                ...
                className="text-xs text-zinc-500 hover:text-zinc-300 font-bold underline ml-2 cursor-pointer"
              >
                Cerrar
              </button>
            </div>
          ...
          {filteredStructuresList.length === 0 ? (
            <div className="p-4 bg-zinc-950 rounded-xl text-center text-xs text-zinc-500 font-medium">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {filteredStructuresList.map((item) => (
                <div
                  key={item.code}
                  className="p-3 bg-zinc-950 border border-zinc-800/80 rounded-xl space-y-2 hover:border-amber-500/30 transition-all"
                >
                  <div className="flex items-center justify-between">
                    <span className="font-mono font-black text-xs text-amber-400 flex items-center gap-1.5">
                      <MapPin className="w-3.5 h-3.5 text-amber-500" />
                      {item.code}
                    </span>
                    <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400">
                      Tipo: {item.type}
                    </span>
                  </div>
                  ...
                  <div className="pt-2 border-t border-zinc-900 space-y-1">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {item.pms.length === 0 ? (
                        <span className="text-[10px] text-zinc-600">Sin PM activo asignado</span>
                      ) : (
                        item.pms.map((pmCode) => (
                          <span
                            key={pmCode}
                            className="text-[10px] font-mono font-bold px-1.5 py-0.5 bg-zinc-900 border border-zinc-800 text-zinc-300 rounded"
                          >
                            {pmCode}
                          </span>
                        ))
      ...
      <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-5 shadow-lg space-y-4">
        <div className="border-b border-zinc-800/60 pb-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            <h3 className="text-sm font-extrabold text-zinc-100 uppercase tracking-wider flex items-center gap-2">
          ...
          <span className="text-xs font-mono font-bold text-zinc-400 bg-zinc-800 px-3 py-1 rounded-lg self-start sm:self-auto">
          ...
          >
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-zinc-200">Programados esta Semana</span>
              </div>
              <span className="text-lg font-black text-emerald-400">{thisWeekList.length}</span>
            </div>
          ...
          >
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-zinc-200">Programados Vencidos</span>
              </div>
              <span className="text-lg font-black text-red-400">{overdueList.length}</span>
            </div>
          ...
          >
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-zinc-200">Sin Programar Aún</span>
              </div>
              <span className="text-lg font-black text-blue-400">{notScheduledList.length}</span>
            </div>
          ...
          >
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-zinc-200">Intervenciones Reprogramadas</span>
              </div>
              <span className="text-lg font-black text-purple-400">{reprogrammedList.length}</span>
            </div>
        ...
        {expandedCategory && (
          <div className="mt-4 pt-4 border-t border-zinc-800 animate-fade-in space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-xs font-extrabold uppercase tracking-wider text-zinc-300 flex items-center gap-1.5">
                {expandedCategory === "thisWeek" && <span className="text-emerald-400">Puntos de Medida Programados para esta Semana ({thisWeekList.length})</span>}
                {expandedCategory === "overdue" && <span className="text-red-400">Puntos de Medida Programados Vencidos sin Intervención ({overdueList.length})</span>}
                {expandedCategory === "notScheduled" && <span className="text-blue-400">Puntos de Medida Afectados sin Fecha Programada ({notScheduledList.length})</span>}
                {expandedCategory === "reprogrammed" && <span className="text-purple-400">Puntos de Medida con Intervenciones Reprogramadas ({reprogrammedList.length})</span>}
              </h4>
              <button 
                onClick={() => setExpandedCategory(null)}
                className="text-[11px] text-zinc-500 hover:text-zinc-300 font-bold underline cursor-pointer"
              >
                Cerrar lista
              </button>
            </div>
              ...
              thisWeekList.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                  {thisWeekList.map((item, idx) => (
                    <div key={idx} className="p-3 bg-zinc-950 border border-zinc-800/80 rounded-xl flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          <span className="font-mono font-bold text-xs text-amber-400">{item.pm.codigoPM}</span>
                          <span className="text-xs font-bold text-zinc-200 truncate">{item.pm.nombrePM}</span>
                        </div>
                        <p className="text-[11px] text-zinc-400 font-medium">{item.failureDetail}</p>
                      ...
                      <button
                        ...
                        className="px-2.5 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 rounded-lg text-[11px] font-bold flex items-center gap-1 shrink-0 cursor-pointer"
                      >
              ...
              overdueList.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                  {overdueList.map((item, idx) => (
                    <div key={idx} className="p-3 bg-zinc-950 border border-red-500/20 rounded-xl flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          <span className="font-mono font-bold text-xs text-red-400">{item.pm.codigoPM}</span>
                          <span className="text-xs font-bold text-zinc-200 truncate">{item.pm.nombrePM}</span>
                        </div>
                        <p className="text-[11px] text-zinc-400 font-medium">{item.failureDetail}</p>
                      ...
                      <button
                        onClick={() => onNavigate("directorio")}
                        className="px-2.5 py-1.5 bg-red-500/20 hover:bg-red-500/30 text-red-300 border border-red-500/30 rounded-lg text-[11px] font-bold flex items-center gap-1 shrink-0 cursor-pointer"
                      >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                  {notScheduledList.map((item, idx) => (
                    <div key={idx} className="p-3 bg-zinc-950 border border-zinc-800/80 rounded-xl flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          <span className="font-mono font-bold text-xs text-blue-400">{item.pm.codigoPM}</span>
                          <span className="text-xs font-bold text-zinc-200 truncate">{item.pm.nombrePM}</span>
                        </div>
                        <p className="text-[11px] text-zinc-400 font-medium">{item.failureDetail}</p>
                        <p className="text-[10px] text-zinc-500 font-bold">Sin fecha asignada ("Sin definir")</p>
                      </div>
                      <button
                        onClick={() => onNavigate("directorio")}
                        className="px-2.5 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 rounded-lg text-[11px] font-bold flex items-center gap-1 shrink-0 cursor-pointer"
              ...
              reprogrammedList.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                  {reprogrammedList.map((item, idx) => (
                    <div key={idx} className="p-3 bg-zinc-950 border border-purple-500/20 rounded-xl flex items-center justify-between gap-3">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-mono font-bold text-xs text-purple-400">{item.pm.codigoPM}</span>
                          <span className="text-xs font-bold text-zinc-200 truncate">{item.pm.nombrePM}</span>
                        ...
                        <p className="text-[10px] text-purple-300 font-bold flex items-center gap-1">
                          <RotateCcw className="w-3 h-3 text-purple-400" /> Reagendada para: {item.dateStr}
                        </p>
                      </div>
                      <button
                        onClick={() => onNavigate("directorio")}
      ...
      <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-6 shadow-lg space-y-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-zinc-800/60 pb-4 gap-3">
          <div>
            <h3 className="text-sm font-extrabold text-zinc-100 uppercase tracking-wider flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-xs font-mono font-bold text-red-400 bg-red-500/10 border border-red-500/20 px-2.5 py-1 rounded-lg">
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Legend 
                ...
                formatter={(value) => <span className="text-xs font-bold text-zinc-300">{value}</span>}
              />
              <Bar 
      ...
      {selectedMonthModal && (
        ...
        >
          <div 
            className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-lg p-5 shadow-2xl space-y-4 max-h-[85vh] flex flex-col"
          ...
          >
            <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
              ...
              <button
                onClick={() => setSelectedMonthModal(null)}
                className="p-1.5 rounded-lg bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-zinc-200 transition-all cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center font-mono text-xs">
              <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-2.5 text-red-400">
                <div className="text-[10px] font-sans font-extrabold text-zinc-400 uppercase">Fallas del Mes</div>
                <div className="text-lg font-black mt-0.5">{selectedMonthModal["Fallas del Mes"]}</div>
              </div>
              <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-2.5 text-emerald-400">
                <div className="text-[10px] font-sans font-extrabold text-zinc-400 uppercase">Corregidos</div>
                <div className="text-lg font-black mt-0.5">{selectedMonthModal["Arreglados del Mes"]}</div>
              </div>
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-2.5 text-amber-400">
                <div className="text-[10px] font-sans font-extrabold text-zinc-400 uppercase">Acumulado</div>
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-1 pt-1">
              ...
              {(!selectedMonthModal.fallasDelMesList || selectedMonthModal.fallasDelMesList.length === 0) ? (
                <div className="p-4 bg-zinc-900/50 rounded-xl text-center text-xs text-zinc-500">
                  No se registraron fallas en este periodo.
                </div>
              ) : (
                selectedMonthModal.fallasDelMesList.map((item: any, idx: number) => (
                  <div key={idx} className="p-3 bg-zinc-900/80 border border-zinc-800 rounded-xl space-y-1 hover:border-zinc-700 transition-all">
                    <div className="flex items-center justify-between">
                    ...
                    <p className="text-xs font-bold text-zinc-200">{item.nombrePM}</p>
                    {(item.componente || item.detalle) && (
                      <p className="text-xs text-red-300/90 bg-red-500/10 border border-red-500/20 px-2.5 py-1 rounded-lg mt-1 font-medium">
