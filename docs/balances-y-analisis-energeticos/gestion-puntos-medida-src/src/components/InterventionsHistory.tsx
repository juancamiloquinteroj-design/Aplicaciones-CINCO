...
export default function InterventionsHistory({ pms, onDeleteIntervention }: InterventionsHistoryProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-zinc-900/50 backdrop-blur-md p-5 rounded-xl border border-zinc-800 shadow-xl space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3 pt-3 border-t border-zinc-800/60 text-xs">
          ...
          <div className="flex items-center gap-1.5">
            <label className="text-zinc-500 font-medium">Desde:</label>
            <input
              type="date"
              value={startDate}
              onChange={(e) => setStartDate(e.target.value)}
              className="px-2.5 py-1.5 bg-zinc-950 border border-zinc-800 rounded-md text-zinc-200 focus:outline-none focus:border-amber-500 font-mono"
            />
          </div>

          <div className="flex items-center gap-1.5">
            <label className="text-zinc-500 font-medium">Hasta:</label>
            <input
              type="date"
              value={endDate}
      ...
      {filteredInterventions.length === 0 ? (
        <div className="text-center py-12 bg-zinc-900/50 rounded-xl border border-zinc-800 shadow-xl animate-pulse">
          <History className="w-10 h-10 text-zinc-700 mx-auto mb-3" />
          <p className="text-sm text-zinc-400 font-medium">No se han registrado intervenciones aún.</p>
        ...
        <div className="space-y-4">
          {filteredInterventions.map(({ pmId, pmCodigo, pmNombre, int }) => (
            <div
              key={int.id}
              className="relative bg-zinc-900/50 backdrop-blur-md p-5 pr-12 md:pr-14 rounded-xl border border-zinc-800 shadow-xl hover:border-zinc-700 transition-all flex flex-col md:flex-row gap-5 group/item"
            >
              {/* Trash/Delete button absolute on top-right with stateful confirm */}
              <div className="absolute top-4 right-4 flex items-center gap-1.5 z-10">
                {confirmDeleteId === int.id ? (
                  <div className="flex items-center gap-1 bg-zinc-950 p-1 rounded-lg border border-red-500/30 shadow-md animate-fade-in">
                    <span className="text-[10px] font-bold text-red-400 px-1.5 uppercase font-mono flex items-center gap-0.5">
                    ...
                    <button
                      ...
                      className="px-2 py-1 text-[10px] font-bold bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded cursor-pointer transition-all uppercase"
                    >
                      No
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    className="p-2 text-zinc-500 hover:text-red-400 bg-zinc-950/60 hover:bg-red-500/10 rounded-lg border border-zinc-800 hover:border-red-500/20 transition-all cursor-pointer shadow-sm md:opacity-0 group-hover/item:opacity-100 focus:opacity-100"
              ...
              <div className="md:w-1/4 border-b md:border-b-0 md:border-r border-zinc-800/80 pb-3 md:pb-0 md:pr-5 flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <span className="text-[9px] bg-emerald-500/15 text-emerald-400 border border-emerald-500/25 font-black px-2 py-0.5 rounded-full uppercase tracking-wider">
                      Solucionado
                    </span>
                    <span className="text-[10px] text-zinc-500 font-mono">ID: {int.id}</span>
                  </div>
                  <h4 className="font-mono font-extrabold text-zinc-100 text-sm leading-tight">
                    PM: {pmCodigo}
                  </h4>
                  <p className="text-xs text-zinc-400 leading-normal">{pmNombre}</p>
                </div>
              ...
              <div className="flex-1 space-y-4">
                ...
                <div className="space-y-1.5">
                  <div className="text-[10px] font-extrabold text-zinc-400 uppercase tracking-wider flex items-center gap-1">
                    <FileText className="w-3.5 h-3.5 text-zinc-500" />
                    Informe de Trabajo / Acción
                  </div>
                  <p className="text-sm text-zinc-300 leading-relaxed font-medium bg-zinc-950 p-3 rounded-lg border border-zinc-800 italic">
                    &ldquo;{int.descripcion}&rdquo;
                  </p>
                </div>

                {/* DETAILS COMPLETED & PENDING */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1.5 border-t border-zinc-800/80">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {int.fallasCorregidas.map((fc) => (
                        <span
                          key={fc.id}
                          className="inline-flex items-center gap-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/15 px-2.5 py-1 rounded-md text-xs font-bold"
                        >
                  ...
                  <div className="space-y-1.5">
                    ...
                    {int.fallasPendientes.length === 0 ? (
                      <p className="text-xs text-emerald-400 font-extrabold bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/15 inline-block uppercase tracking-wider">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {int.fallasPendientes.map((fp) => (
                          <span
                            key={fp.id}
                            className="inline-flex items-center gap-1 bg-amber-500/10 text-amber-400 border border-amber-500/15 px-2.5 py-1 rounded-md text-xs font-bold"
                          >
